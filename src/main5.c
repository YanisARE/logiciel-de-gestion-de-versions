...
int main(){
    ...
    printf("Le mode de test est %o\n",mode);
    if (wt!=NULL){
        appendWorkTree(wt, "TestFiles/test1appendworktree.txt", "abcd1234", mode); 
        appendWorkTree(wt, "TestFiles/test2appendworktree.txt", "abcd246", mode);
    }
    
    char* saved = saveWorkTree(wt, "TestFiles/saveWorkTreetmp.txt");
